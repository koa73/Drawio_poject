...
import {createSeafPluginService} from './seaf/seafPluginService.js';
...
const seafTerminalDirUrl = url.pathToFileURL(path.join(__dirname, 'seaf')).href.replace(/\/.\:\//, str => str.toUpperCase());
const seafTerminalXtermJsPath = require.resolve('@xterm/xterm/lib/xterm.js');
const seafTerminalFitAddonJsPath = require.resolve('@xterm/addon-fit/lib/addon-fit.js');
const seafTerminalXtermCssPath = require.resolve('@xterm/xterm/css/xterm.css');
const seafTerminalAllowedDependencyPaths = new Set([
	path.resolve(seafTerminalXtermJsPath),
	path.resolve(seafTerminalFitAddonJsPath),
	path.resolve(seafTerminalXtermCssPath)
]);
const interactiveTerminalSessions = new Map();

function resolveSeafRuntimeDefaultsDir()
{
	const candidates = [
		seafRuntimeDefaultsDir,
		path.join(appBaseDir, 'resources', 'seaf-runtime-default'),
		path.join(process.resourcesPath || '', 'seaf-runtime-default')
	].filter((entry) => typeof entry === 'string' && entry.length > 0);

	for (const candidate of candidates)
	{
		if (fs.existsSync(path.join(candidate, seafPluginFileName)) &&
			fs.existsSync(path.join(candidate, seafRuntimeDirName)))
		{
			return candidate;
		}
	}
...
{
	const normalized = frame.url.replace(/\/.\:\//, str => str.toUpperCase());
	return normalized.startsWith(codeUrl) || normalized.startsWith(seafTerminalDirUrl);
}
...
app.whenReady().then(() =>
{
	ensureSeafRuntimeInstalled().then((result) =>
	{
		if (result != null && result.ok === true)
		{
			console.log('[SEAF bootstrap] runtime ready', result);
		}
		else
		{
			console.warn('[SEAF bootstrap] runtime not installed', result);
		}
	}).catch((e) =>
	{
		console.error('[SEAF bootstrap] runtime bootstrap failed:', e);
	});
	...
	{
		const currentUrl = details.url.replace(/\/.\:\//, str => str.toUpperCase());
		const allowedSeafUrl = isSeafPluginUrl(details.url);
		const allowedSeafTerminalAsset = currentUrl.startsWith(seafTerminalDirUrl);
		const allowedSeafTerminalDependency = isAllowedSeafTerminalDependencyUrl(details.url);

		if (!currentUrl.startsWith(codeUrl) &&
			!allowedSeafTerminalAsset &&
			!allowedSeafTerminalDependency &&
			!allowedSeafUrl &&
			(!isPluginsEnabled() || (isPluginsEnabled() && !currentUrl.startsWith(pluginsCodeUrl))))
		{
			console.log('Blocked loading file from ' + details.url, currentUrl, codeUrl, pluginsCodeUrl);
...
{
	return path.join(getAppDataFolder(), 'plugins');
}

function normalizePluginRequestPath(plugin)
{
	...
	let normalized = plugin.trim().replace(/\\/g, '/');
...
function isSeafRuntimePath(requestPath)
{
	const normalized = normalizePluginRequestPath(requestPath);
	if (normalized == null || normalized.length === 0 || normalized.indexOf('..') >= 0)
	{
		return false;
	}
...
function getSeafRuntimeTargets()
{
	const pluginsDir = getPluginsDir();
	return {
		pluginsDir,
		pluginFile: path.join(pluginsDir, seafPluginFileName),
		runtimeDir: path.join(pluginsDir, seafRuntimeDirName)
	};
}

function getSeafRuntimeDefaults()
{
	const defaultsDir = resolveSeafRuntimeDefaultsDir();
	return {
		defaultsDir,
...
function isSeafPluginUrl(fileUrl)
{
	...
	try
	{
		...
		return isSeafRuntimePath(relativePath);
	}
	catch (e)
	{
		return false;
...
function isAllowedSeafTerminalDependencyUrl(fileUrl)
{
	if (typeof fileUrl !== 'string')
	{
		return false;
	...
	{
		const filePath = path.resolve(url.fileURLToPath(fileUrl));
		return seafTerminalAllowedDependencyPaths.has(filePath);
	}
	catch (e)
	{
		return false;
...
async function ensureSeafRuntimeInstalled()
{
	...
	console.log('[SEAF bootstrap] started', details);

	if (!fs.existsSync(defaults.pluginFile))
	{
		return {
			ok: false,
			reason: 'defaults_plugin_missing',
			details
		};
	}

	if (!fs.existsSync(defaults.runtimeDir))
	{
		return {
			ok: false,
			reason: 'defaults_runtime_missing',
			details
		};
	}

	await fsProm.mkdir(targets.pluginsDir, {recursive: true});
	console.log('[SEAF bootstrap] plugins dir ensured', {
		pluginsDir: targets.pluginsDir
	});

	if (!fs.existsSync(targets.pluginFile))
	{
		await fsProm.copyFile(defaults.pluginFile, targets.pluginFile);
		console.log('[SEAF bootstrap] plugin file copied', {
			from: defaults.pluginFile,
			to: targets.pluginFile
		});
	}
	else
	{
		console.log('[SEAF bootstrap] plugin file already exists', {
			pluginFile: targets.pluginFile
		});
	}

	if (!fs.existsSync(targets.runtimeDir))
	{
		await fsProm.cp(defaults.runtimeDir, targets.runtimeDir, {recursive: true, force: true});
		console.log('[SEAF bootstrap] runtime directory copied', {
			from: defaults.runtimeDir,
...
const seafPluginService = createSeafPluginService({getAppDataFolder});

function sendToWebContents(contents, channel, payload)
{
...
function getInteractiveTerminalWindowSize(parentWindow)
{
	const bounds = parentWindow != null && !parentWindow.isDestroyed() ?
		parentWindow.getContentBounds() : {width: 1200, height: 800};

	return {
		width: Math.max(640, Math.floor(bounds.width / 2)),
		height: Math.max(360, Math.floor(bounds.height / 2))
	};
}

function appendInteractiveTerminalBuffer(session, chunk)
{
	session.outputBuffer = (session.outputBuffer || '') + String(chunk || '');

	if (session.outputBuffer.length > 200000)
	{
		session.outputBuffer = session.outputBuffer.slice(-200000);
	}
}

function notifyInteractiveSessionOwner(session, type, details = {})
{
	sendToWebContents(session.ownerContents, 'seafInteractiveSessionEvent', Object.assign({
...
function notifyInteractiveTerminalWindow(session, channel, payload)
{
	if (session.terminalWindow != null && !session.terminalWindow.isDestroyed())
	{
		sendToWebContents(session.terminalWindow.webContents, channel, Object.assign({
			sessionId: session.sessionId
		}, payload || {}));
	}
}

function logInteractiveTerminalEvent(session, level, message, data = {})
{
	if (session == null || !session.configPath)
	{
		return;
	}

	seafPluginService.writeClientLog({
...
function killInteractiveSessionPty(session)
{
	if (session == null || session.pty == null || session.ptyKilled)
	{
		return;
	}

	session.ptyKilled = true;

	try
	{
		session.pty.kill();
		logInteractiveTerminalEvent(session, 'warn', 'Interactive terminal PTY killed');
	}
	catch (e)
	{
		// ignore best-effort kill errors
		logInteractiveTerminalEvent(session, 'error', 'Interactive terminal PTY kill failed', {
			error: e.message
		});
	}
}

function cleanupInteractiveTerminalSession(session, reason)
{
	if (session == null || session.cleanedUp)
	{
		return;
	}

	session.cleanedUp = true;
	interactiveTerminalSessions.delete(session.sessionId);

	if (!session.processExited)
	{
		session.status = reason || 'aborted';
		killInteractiveSessionPty(session);
	}

	notifyInteractiveSessionOwner(session, 'terminal-closed', {
		reason: reason || session.status || 'closed',
		status: session.status || 'closed',
		exitCode: session.exitCode,
		exitSignal: session.exitSignal,
		errorMessage: session.errorMessage || null
	});
	logInteractiveTerminalEvent(session, 'info', 'Interactive terminal session cleaned up', {
		reason: reason || session.status || 'closed',
		exitCode: session.exitCode,
		exitSignal: session.exitSignal,
		errorMessage: session.errorMessage || null
...
function extractInteractiveTerminalErrorMessage(session, exitCode)
{
	const fallback = `Interactive terminal process exited with code ${exitCode}`;
	const buffer = (session && typeof session.outputBuffer === 'string') ? session.outputBuffer : '';
	...
	const ansiStripped = buffer.replace(/\x1B\[[0-?]*[ -/]*[@-~]/g, '');
	...
	{
		const line = lines[i];
		if (/exception|error|traceback/i.test(line))
		{
			return line;
		}
	}
...
function createInteractiveTerminalWindow(session)
{
	const size = getInteractiveTerminalWindowSize(session.ownerWindow);
	const win = new BrowserWindow({
		parent: session.ownerWindow,
		modal: true,
		show: false,
		resizable: true,
		autoHideMenuBar: true,
		backgroundColor: '#111111',
		title: session.title || 'SEAF Interactive Terminal',
		width: size.width,
		height: size.height,
		minWidth: 520,
		minHeight: 320,
		webPreferences: {
			preload: seafTerminalPreloadPath,
			contextIsolation: true,
			sandbox: false,
			spellcheck: false
		}
	});
	const logWindowEvent = (level, message, extra = {}) =>
	{
		logInteractiveTerminalEvent(session, level, message, extra);
	};

	logWindowEvent('info', 'Interactive terminal window created', {
		width: size.width,
		height: size.height
	});
	...
	{
		win.show();
		logWindowEvent('info', 'Interactive terminal window ready-to-show');
		notifyInteractiveSessionOwner(session, 'terminal-opened', {
	...
	{
		logWindowEvent('info', 'Interactive terminal webContents did-finish-load');
	});

	win.webContents.on('did-fail-load', (evt, errorCode, errorDescription, validatedURL, isMainFrame) =>
	{
		logWindowEvent('error', 'Interactive terminal webContents did-fail-load', {
			errorCode,
			errorDescription,
			validatedURL,
	...
	{
		logWindowEvent('error', 'Interactive terminal render-process-gone', {
	...
	win.on('unresponsive', () =>
	{
		logWindowEvent('warn', 'Interactive terminal window became unresponsive');
	});

	win.webContents.on('console-message', (evt, level, message, line, sourceId) =>
	{
		logWindowEvent(level >= 2 ? 'warn' : 'info', 'Interactive terminal renderer console message', {
			level,
			line,
			sourceId,
			message
	...
	win.on('closed', () =>
	{
		const reason = session.status || (session.processExited ? 'completed' : 'aborted');
		logWindowEvent('info', 'Interactive terminal window closed', {
			reason
		});
		cleanupInteractiveTerminalSession(session, reason);
	});

	win.loadFile(seafTerminalWindowHtmlPath, {
		query: {
			sessionId: session.sessionId,
			xtermJsUrl: url.pathToFileURL(seafTerminalXtermJsPath).href,
			fitAddonJsUrl: url.pathToFileURL(seafTerminalFitAddonJsPath).href,
			xtermCssUrl: url.pathToFileURL(seafTerminalXtermCssPath).href
		}
	});
...
async function startSeafInteractiveTerminalSession(event, args)
{
	const ownerContents = event.sender;
	const ownerWindow = BrowserWindow.fromWebContents(ownerContents);

	if (ownerWindow == null)
	{
		throw new Error('Unable to resolve owner window for interactive terminal session');
	}

	const prepared = await seafPluginService.prepareInteractiveTerminalCommand(args);
	...
	await seafPluginService.writeClientLog({
		configPath: prepared.configPath,
		level: 'info',
		message: 'Interactive terminal session starting',
		data: {
			commandId: prepared.commandId,
			sessionId
		}
	...
	try
	{
		session.pty = nodePty.spawn(prepared.pythonExe, prepared.args, {
			name: 'xterm-color',
			cwd: prepared.cwd,
			env: prepared.env,
			cols: 80,
		...
		session.terminalWindow = createInteractiveTerminalWindow(session);
		logInteractiveTerminalEvent(session, 'info', 'Interactive terminal PTY spawned', {
		...
		session.pty.onData((data) =>
		{
			...
			if (!firstChunkLogged)
			{
				...
				logInteractiveTerminalEvent(session, 'info', 'Interactive terminal PTY produced first output chunk', {
					chunkLength: typeof data === 'string' ? data.length : 0
				});
			}

			appendInteractiveTerminalBuffer(session, data);
			notifyInteractiveTerminalWindow(session, 'seafInteractiveTerminalData', {
				data
			});
		});
		...
		session.pty.onExit(({exitCode, signal}) =>
		{
			...
			session.errorMessage = exitCode === 0 ? null : extractInteractiveTerminalErrorMessage(session, exitCode);
			logInteractiveTerminalEvent(session, 'info', 'Interactive terminal PTY exited', {
				exitCode,
				signal
			});
			notifyInteractiveTerminalWindow(session, 'seafInteractiveTerminalExit', {
				exitCode,
				signal
			});
			notifyInteractiveSessionOwner(session, 'process-exit', {
				status: session.status,
				exitCode,
				exitSignal: signal,
				errorMessage: session.errorMessage || null
			...
			seafPluginService.writeClientLog({
				configPath: prepared.configPath,
				level: 'info',
				message: 'Interactive terminal session process exited',
				data: {
					commandId: prepared.commandId,
					sessionId,
					exitCode,
			...
			if (session.status === 'failed' && session.terminalWindow != null && !session.terminalWindow.isDestroyed())
			{
				setTimeout(() =>
				{
					if (session.terminalWindow != null && !session.terminalWindow.isDestroyed())
					{
						session.terminalWindow.close();
					}
				}, 120);
			}
		});
	...
	catch (e)
	{
		...
		if (session.terminalWindow != null && !session.terminalWindow.isDestroyed())
		{
			session.terminalWindow.destroy();
		}
		interactiveTerminalSessions.delete(sessionId);
		await seafPluginService.writeClientLog({
			configPath: prepared.configPath,
			level: 'error',
			message: 'Interactive terminal session failed to start',
			data: {
				commandId: prepared.commandId,
				sessionId,
				error: e.message
...
function getInteractiveTerminalSessionSnapshot(sessionId)
{
...
function writeInteractiveTerminalInput(args)
{
	const session = interactiveTerminalSessions.get(args.sessionId);

	if (session == null || session.cleanedUp)
	{
		return {status: 'not_found', sessionId: args.sessionId || null};
	}

	if (session.processExited || session.pty == null)
	{
		return {status: 'completed', sessionId: args.sessionId};
	}

	const data = typeof args.data === 'string' ? args.data : String(args.data || '');
	session.pty.write(data);
	logInteractiveTerminalEvent(session, 'info', 'Interactive terminal input forwarded', {
		length: data.length
	});
	return {status: 'ok', sessionId: args.sessionId};
}

function resizeInteractiveTerminal(args)
{
	const session = interactiveTerminalSessions.get(args.sessionId);
	...
	const cols = Math.max(2, Math.round(Number(args.cols) || 0));
	const rows = Math.max(1, Math.round(Number(args.rows) || 0));
	session.pty.resize(cols, rows);
	logInteractiveTerminalEvent(session, 'info', 'Interactive terminal resized', {
		cols,
		rows
	});
	return {status: 'ok', sessionId: args.sessionId, cols, rows};
}

function closeInteractiveTerminalSession(args)
{
	const session = interactiveTerminalSessions.get(args.sessionId);
	...
	{
		cleanupInteractiveTerminalSession(session, session.processExited ? 'completed' : 'aborted');
	}

	return {status: 'closing', sessionId: args.sessionId};
}

function reportInteractiveTerminalRendererEvent(args)
{
	...
	if (session == null || session.cleanedUp)
	{
		return {status: 'not_found', sessionId: sessionId};
	}

	const level = (typeof args.level === 'string' && args.level.length > 0) ? args.level : 'info';
	const eventName = (typeof args.event === 'string' && args.event.length > 0) ? args.event : 'unknown';
	const details = (args && typeof args.details === 'object' && args.details != null) ? args.details : {};
	logInteractiveTerminalEvent(session, level, 'Interactive terminal renderer event', {
		event: eventName,
		details
	});

	return {status: 'ok', sessionId: sessionId, event: eventName};
}
...
async function getPluginFile(plugin)
{
	const normalized = normalizePluginRequestPath(plugin);
	...
	const prefix = path.resolve(getPluginsDir());
	const pluginFile = path.join(prefix, normalized);

	if (isSeafRuntimePath(normalized) && !fs.existsSync(pluginFile))
	{
		try
		{
			const bootstrapResult = await ensureSeafRuntimeInstalled();
			console.log('[SEAF bootstrap] lazy bootstrap from getPluginFile', {
				plugin: normalized,
		...
		{
			console.error('[SEAF bootstrap] lazy bootstrap failed', {
				plugin: normalized,
...
{
	const pluginFile = await getPluginFile(plugin);
...
ipcMain.on("rendererReq", async (event, args) =>
{
	...
	try
	{
		...
		case 'isFullscreen':
			...
			break;
		case 'getSeafPluginConfig':
			ret = await seafPluginService.loadConfig(args.configPath);
			break;
		case 'runSeafPluginCommand':
			ret = await seafPluginService.runCommand(args);
			break;
		case 'startSeafInteractiveTerminalSession':
			ret = await startSeafInteractiveTerminalSession(event, args);
			break;
		case 'getSeafInteractiveTerminalSnapshot':
			ret = getInteractiveTerminalSessionSnapshot(args.sessionId);
			break;
		case 'writeSeafInteractiveTerminalInput':
			ret = writeInteractiveTerminalInput(args);
			break;
		case 'resizeSeafInteractiveTerminal':
			ret = resizeInteractiveTerminal(args);
			break;
		case 'closeSeafInteractiveTerminalSession':
			ret = closeInteractiveTerminalSession(args);
			break;
		case 'reportSeafInteractiveTerminalRendererEvent':
			ret = reportInteractiveTerminalRendererEvent(args);
			break;
		case 'updateSeafPluginRuntime':
			ret = await seafPluginService.updateRuntime(args);
			break;
		case 'pollSeafPluginJob':
			ret = seafPluginService.pollJob(args.jobId);
			break;
		case 'cancelSeafPluginJob':
			ret = seafPluginService.cancelJob(args.jobId);
			break;
		...
		case 'startSeafManualIndicator':
			...
			break;
		case 'finishSeafManualIndicator':
			ret = seafPluginService.finishManualIndicator(args);
			break;
		case 'getSeafEnvConfig':
			ret = await seafPluginService.getEnvConfig(args);
			break;
		...
		case 'saveSeafEnvConfig':
			...
			break;
		case 'selectSeafEnvFile':
			dialogOpen = true;
			try
			{
				const defaultPath = (typeof args.defaultPath === 'string' && args.defaultPath.trim().length > 0) ?
					args.defaultPath.trim() : undefined;
				const filters = Array.isArray(args.filters) ? args.filters : [];
				const properties = Array.isArray(args.properties) && args.properties.length > 0 ?
					args.properties : ['openFile'];
				ret = await showOpenDialog(defaultPath, filters, properties);
				ret = ret && ret.filePaths && ret.filePaths.length > 0 ? ret.filePaths[0] : null;
			}
			catch (e)
			{
				throw new Error('Failed to open file picker: ' + (e && e.message ? e.message : String(e)));
			}
			finally
			{
				dialogOpen = false;
