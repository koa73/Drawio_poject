...
- назначение `OID` при добавлении фигур выполняется через event-маршрут `seaf.company.ta.*` и Python handler (без прямого UI-автогенератора).
...
- уникальность `OID` контролируется строго в рамках текущей диаграммы; при import-коллизиях показывается таблица конфликтов для ручного разрешения (без автодедупликации).
...
- для P41-стенсилов добавлен собственный диалог `SeafEditDataDialog`, заменяющий штатный «Edit Data» drawio: per-schema настройки `edit_data` (`seaf|standard|both`) и `data_lock` (список защищённых атрибутов) задаются в `seaf-plugin-runtime/conf/stencils/config.yaml`. По умолчанию для всех schemas защищены `OID` и `schema` (запрет редактирования, удаления и добавления свойства с защищённым именем). Реинжиниринг v2 разделяет решение на слои `EditDataModeEngine`, `ContextMenuPresenter`, `EditDataDialogRouter`, `EditDataSessionCoordinator` с единым DTO `EditDataIntent` (`schema/mode/lockList/policySource`). Каноническая маршрутизация через override `EditorUi.prototype.showDataDialog` покрывает Format panel, Ctrl+M и штатный action `editData`; в контекстном меню контракт детерминирован и без дублей: `seaf` — только SEAF, `both` — один standard + один SEAF, `standard` — один standard + SEAF disabled. Plugin больше не добавляет стандартный `Edit Data` вручную: единственный источник standard-пункта — базовый popup draw.io. Матрица применяется только для `policySource=config-hit` (schema найдена в `stencils/config.yaml`). Для grouped stencil-элементов mode и target Edit Data резолвятся по ближайшему родителю со `schema`, поэтому RMB-клик в дочерний служебный `mxCell` не ломает логику. Загрузка stencil-policy выполняется через typed IPC action `getSeafStencilConfig` (legacy `readSeafPluginFile` оставлен как migration fallback под feature flag). Fallback: если `conf/stencils/config.yaml` не загрузился или схема не описана, plugin резолвит `mode=seaf` и `data_lock=[OID, schema]` для любого ключа с префиксом `seaf.`. Для диагностики нужен `pluginLogLevel=info|debug` (при `none` диагностические debug/info записи не выводятся). Резерв `schemas.<x>.fields` под Phase 2 rich-виджеты (combo/radio/checkbox).
...
- `Создать страницу` расширена mirror-сценарием: после успешного `createPage + setCellLinkToPage` runtime переключается на новую страницу, ищет mirror-элемент по `schemas.<sourceSchema>.mirror` в `stencils/config.yaml`, вставляет его из библиотеки `SEAF_Р41`, синхронизирует `data` (copy-all) с исходным объектом и назначает слой через тот же Python layer-routing helper, что используется в `all_add` (без JS schema->layer fallback); неуспех вставки/резолва слоя дает сообщение `Не возможно добавить элемент <mirror> на страницу` и расширенную диагностику в `seaf-plugin.log`.
- Последней командой `add_page` теперь выполняется автозаполнение пустых `OID` на созданной странице: runtime выбирает объекты, где атрибут `OID` существует, но пуст, и назначает уникальные значения по тому же алгоритму, что и в `all_add` (`<companyPrefix>.<schemaCode>.<sequence>`).
